...
<template>
  <div>
    ...
    <Button type="primary"
            @click="unfreezeSecondaryAccountsFun">解冻所有辅助账户 unfreezeSecondaryAccounts</Button>
    <Button type="primary"
            @click="setSigningAccountFun">设置签名账户 setSigningAccount</Button>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    async setSigningAccountFun () {
      // const account = await this.apiAlice.accountManagement.getAccount({ address: '5HR15dJ26BfbQAxBpNYzzNKqZfHQeS12h4jmBZzg8r4B4QpS' })
      // const aa = await this.apiAlice.setSigningAccount(account)
      const aa = await this.apiAlice.setSigningAccount('5HR15dJ26BfbQAxBpNYzzNKqZfHQeS12h4jmBZzg8r4B4QpS')
      console.log(aa)
    ...
    async unfreezeSecondaryAccountsFun () {
      // 解冻
    ...
    async freezeSecondaryAccountsFun () {
      // 冻结
    ...
    async subsidizeAccountReceiveFun () {
      // 补贴账户接受
      // 辅助账户的信息
      if (!this.assitAccount) {
        this.assitAccount = await this.connect(this.assitAccountKey, 2);
    ...
    async removeSecondaryAccountsFun () {
      // 删除辅助账户
      // 辅助账户信息：
      if (this.assitAccount) {
        this.assitAccount = await this.connect(this.assitAccountKey, 2);
    ...
    async getAccountBalance () {
      const getAccountBalance = await this.apiAlice.accountManagement.getAccountBalance(
        "0x1a3fb688e10f4a544ab627c74b4059133ac81cc8a09cb947dbb65ff55d56c975"
      );
      console.log(
        "账户自由资金：",
        getAccountBalance.free.toString(),
        "账户冻结资金：",
        getAccountBalance.locked.toString(),
        "账户总额：",
        getAccountBalance.total.toString()
      );
